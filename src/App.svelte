...
<script>
  ...
  //     .getVoices()
  //     .filter((v) => v.lang.startsWith("es"))[0];
  // };
  let voiceList = window.speechSynthesis
    .getVoices()
    .filter(
      (v) =>
        v.lang.startsWith("es-MX") &&
        !v.voiceURI.includes("Spanish") &&
        !v.voiceURI.includes("eloquence")
    );

  let voice = voiceList[voiceList.length - 1];

  if (!voice) {
    voiceList = window.speechSynthesis
      .getVoices()
      .filter((v) => v.lang.startsWith("es"));

    voice = voiceList[voiceList.length - 1];
  }
  // debugger;
  ...
  (async () => {
    ...
    console.log(seed);
    let a = await fetch(
      "https://tatoeba.elnu.com/?from=spa&orphans=no&sort=random&to=eng&trans_filter=limit&trans_to=eng&unapproved=no&limit=1&rand_seed=" +
    ...
    rubyTexts = spanishText.split(" ").map((x) => {
      return {
        word: x,
      };
    });
...
{#if !spanishText}
  <p>Generating Spanish Sentence</p>
{:else}
  <div id="container">
    <!-- <p id="cmnText" />
  ...
  {/if} -->
    <!-- <div id="ruby" /> -->
    <!-- <p id="spanishText">{spanishText}</p> -->
    ...
    {/if} -->
      {#if rubyTexts.length > 0}
        ...
        {/each}
      {:else}
        <small>Generating Spanish Sentence...</small>
      {/if}
    </div>
  </div>
{/if}
...
<button
  ...
  on:click={() => {
    navigator.clipboard.writeText(spanishText);
  }}>Copy Spanish Sentence</button
>
